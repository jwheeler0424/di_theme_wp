...
class TaxonomyCallbacks
{
    ...
    {
        echo 'Create the Custom Taxonomies.';
    }

    public function taxSanitize( $input )
    {
        $output = get_option( 'di_plugin_tax' );
    ...
    public function checkboxPostTypesField( $args )
    {
        ...
        foreach ($post_types as $post) {
            ...
            $output .= '<div class="'. $classes .' mb-10"><input type="checkbox" id="'. $post .'" name="'. $option_name .'['. $name .']['. $post .']" value="1" class="'. $classes .'"'. ( $checked ? 'checked' : '') .'><label for="'. $post .'"><div></div></label> <strong>'. $post .'</strong></div>';
        }
