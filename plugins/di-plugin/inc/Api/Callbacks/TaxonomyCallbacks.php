...
class TaxonomyCallbacks
{
    ...
    public function checkboxPostTypesField( $args )
    {
        ...
        foreach ($post_types as $post) {
            ...
            $output .= '<div class="'. $classes .' mb-10"><input type="checkbox" id="'. $post .'" name="'. $option_name .'['. $name .']['. $post .']" vale="1" class="'. $classes .'"'. ( $checked ? 'checked' : '') .'><label for="'. $post .'"><div></div></label> <strong>'. $post .'</strong></div>';
        }
